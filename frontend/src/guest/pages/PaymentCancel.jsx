...
const PaymentCancel = () => {
  ...
  const { selectedBooking: booking, loading } = useSelector(
  ...
  const handleRetryPayment = () => {
    if (booking?.room_id) {
      navigate(`/book/${booking.room_id}?retry=${bookingId}`);
    } else {
      navigate("/");
    }
  };
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-4">
      <Card className="max-w-lg text-center">
        ...
        <h1 className="mt-6 text-2xl font-bold text-ink dark:text-dark-ink">Payment Cancelled</h1>
        ...
        {booking && (
          <div className="mt-6 rounded-xl border border-border bg-surface/50 p-4 text-left dark:border-dark-border dark:bg-dark-surface/50">
            <p className="text-sm font-semibold text-ink dark:text-dark-ink">Pending Booking</p>
            <div className="mt-3 space-y-2 text-sm">
              ...
              <div className="flex justify-between border-t border-border pt-2 dark:border-dark-border">
                ...
                <span className="font-medium text-amber-600 dark:text-amber-400">Payment Pending</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <Button onClick={handleRetryPayment} className="w-full sm:w-auto">Try Payment Again</Button>
          <Link to="/my-bookings"><Button variant="outline" className="w-full sm:w-auto">View My Bookings</Button></Link>
        </div>
