...
export default function AdminDashboard() {
  ...
  const loading = bookingsLoading || roomsLoading || usersLoading;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Welcome back! Here's what's happening with your properties.</p>
      </div>

      {/* Stats Grid */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Total Bookings" value={stats.totalBookings} subtitle={`${stats.upcomingBookings} upcoming`} color="brand" icon={<svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>} />
        <StatCard title="Total Revenue" value={formatPrice(stats.totalRevenue)} subtitle="All time" color="green" icon={<svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
        <StatCard title="Total Users" value={stats.totalUsers} subtitle="Registered accounts" color="purple" icon={<svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>} />
        <StatCard title="Total Rooms" value={stats.totalRooms} subtitle="Listed properties" color="orange" icon={<svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>} />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Link to="/admin/users" className="flex items-center gap-4 rounded-2xl border border-border bg-panel p-4 shadow-sm transition-colors hover:border-purple-200 hover:bg-purple-50 dark:hover:bg-purple-900/20">
          ...
          <div><p className="font-semibold text-ink">Manage Users</p><p className="text-sm text-muted">View user profiles and details</p></div>
        </Link>
        <Link to="/admin/rooms" className="flex items-center gap-4 rounded-2xl border border-border bg-panel p-4 shadow-sm transition-colors hover:border-orange-200 hover:bg-orange-50 dark:hover:bg-orange-900/15">
          ...
          <div><p className="font-semibold text-ink">Manage Rooms</p><p className="text-sm text-muted">Add, edit, or remove listings</p></div>
        </Link>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-panel shadow-sm">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <Link to="/admin/bookings" className="text-sm font-medium text-brand-600 hover:text-brand-700">View all</Link>
        </div>
        <div className="overflow-x-auto">
          {recentBookings.length === 0 ? (
            <div className="py-12 text-center"><p className="text-sm text-muted">No bookings yet</p></div>
          ) : (
                    ...
                    <tr key={booking.id} className="border-b border-border last:border-0">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {booking.room?.image && <img src={booking.room.image} alt={booking.room?.title || "Room"} className="h-10 w-10 rounded-lg object-cover" />}
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <p className="text-sm text-ink">{booking.user_full_name || booking.user?.full_name || "Guest"}</p>
                        <p className="text-xs text-muted">{booking.user?.email || booking.user_email || "N/A"}</p>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-ink">{new Date(booking.booking_date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${statusColor}`}>{statusText}</span>
                      </td>
