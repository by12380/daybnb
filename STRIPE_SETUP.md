## Stripe payments (Supabase + Vite) setup

This project uses **Stripe Checkout** + **Supabase Edge Functions**:

- `create-checkout-session`: creates a `bookings` row with `status = "payment_pending"` and redirects to Stripe Checkout
- `confirm-checkout-session`: called from `/payment/success` to verify the Checkout Session and set `status = "pending"` (awaiting admin approval)
- `stripe-webhook` (recommended for production): updates bookings even if the user closes the browser
...
- **Webhook signing secret** → `STRIPE_WEBHOOK_SECRET` (after you create the webhook endpoint)
...
- **Project Settings → Functions → Secrets**
  - add:
    - `STRIPE_SECRET_KEY`
    - `STRIPE_WEBHOOK_SECRET` (only needed if you enable webhooks)
    - `PUBLIC_SITE_URL` (optional but recommended for local dev, e.g. `http://localhost:5173`)

### 3) Deploy the Edge Functions

From the repo root (requires Supabase CLI and you being logged in):
...
### 4) Configure Stripe webhook (recommended)

In Stripe Dashboard → **Developers → Webhooks**:

- Add endpoint pointing to your Supabase function URL for `stripe-webhook`
- Select event:
  - `checkout.session.completed`
- Copy the signing secret into Supabase as `STRIPE_WEBHOOK_SECRET`

### 5) Booking/payment status flow

- User books → Edge Function inserts booking with **`status = "payment_pending"`** and sends user to Stripe
- Stripe success → `/payment/success` calls `confirm-checkout-session`
- After confirmation → booking becomes **`status = "pending"`** (awaiting admin approval)
- Admin approves → booking becomes **`status = "approved"`**

### Notes

- This code assumes your `bookings` table allows `status` values including `"payment_pending"`.
- No new DB columns are required (we use Stripe `client_reference_id`/`metadata` to map back to `booking.id`).
